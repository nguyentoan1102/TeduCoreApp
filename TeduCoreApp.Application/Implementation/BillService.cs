...
namespace TeduCoreApp.Application.Implementation
{
    public class BillService : IBillService
    {
        ...
        public BillService(IBillRepository orderRepository, IBillDetailRepository orderDetailRepository, IColorRepository colorRepository, IProductRepository productRepository, ISizeRepository sizeRepository, IUnitOfWork unitOfWork)
        {
        ...
        {
            Bill bill = Mapper.Map<BillViewModel, Bill>(billVm);
            List<BillDetail> billDetails = Mapper.Map<List<BillDetailViewModel>, List<BillDetail>>(billVm.BillDetails);
            foreach (var item in billDetails)
            {
                item.Price = _productRepository.FindById(item.ProductId).Price;
            }
            bill.BillDetails = billDetails;
            _orderRepository.Add(bill);
        }
        ...
        public void Update(BillViewModel billVm)
        {
            ...
            var addedDetails = order.BillDetails.Where(x => x.Id == 0).ToList();
            var updateDetails = order.BillDetails.Where(x => x.Id != 0).ToList();
